...
import { mountHQOffice, renderHQScene, resetOfficeAssetCache } from "./renderScene";
...
export function HQCanvas() {
  ...
  return (
    <div className="canvas-stack">
      ...
      {renderError && <div className="canvas-error" role="alert"><strong>직원 이미지를 복구하지 못했습니다.</strong><span>{renderError}</span><button onClick={() => { resetOfficeAssetCache(); setRenderNonce((value) => value + 1); }}>이미지 다시 불러오기</button></div>}
      <p className="sr-only" aria-live="polite">
        {snapshot ? `${snapshot.agents.length}명 중 현재 층 ${visibleAgents.length}명이 표시됩니다. ${matchingAgents.length > MAX_FLOOR_AGENTS ? `${MAX_FLOOR_AGENTS}명 초과 실행은 현재 필터의 대표 좌석을 표시하며 명부에서 전원을 검색할 수 있습니다.` : ""}` : "회사 데이터를 불러오는 중입니다."}
      </p>
      {snapshot && <div className="floor-density" aria-hidden="true">현재 층 <strong>{visibleAgents.length}명</strong> · 전체 <strong>{snapshot.agents.length}명</strong>{matchingAgents.length > MAX_FLOOR_AGENTS ? " · 집계 표시" : ""}</div>}
